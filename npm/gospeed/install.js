...
// matching this package's version, extract just the `gospeed` binary, and
// write a JS shim so `npm i -g @goozt/gospeed` produces a working `gospeed`
// command.
//
// Avoids any runtime deps so it works during a fresh `npm install`.
...
async function main() {
  ...
  // Remove sibling binaries we don't ship in this package — the archive
  // bundles every binary; this package only exposes the ones in BINARIES.
  const keep = new Set(
